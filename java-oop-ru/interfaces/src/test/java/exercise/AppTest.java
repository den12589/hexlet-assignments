...
class AppTest {
    ...
    void reversedSequenceTest(){
        var result = new ReversedSequence("asdFg");
        assertThat(result.toString()).isEqualTo("gFdsa");
        assertThat(result.charAt(1)).isEqualTo('F');
        assertThat(result.length()).isEqualTo(5);
        assertThat(result.subSequence(2,5)).isEqualTo("dsa");
    }
